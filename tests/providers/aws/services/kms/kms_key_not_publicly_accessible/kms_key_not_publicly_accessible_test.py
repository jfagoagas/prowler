...
class Test_kms_key_not_publicly_accessible:
    ...
    @mock_aws
    def test_kms_key_public_accessible_with_describe_key_action(self):
        """Test that KMS key with public principal and kms:DescribeKey is flagged as publicly accessible"""
        # Generate KMS Client
        kms_client = client("kms", region_name=AWS_REGION_US_EAST_1)
        # Create KMS key with public policy granting kms:DescribeKey
        key = kms_client.create_key(
            MultiRegion=False,
            Policy=json.dumps(
                {
    ...
    @mock_aws
    def test_kms_key_public_accessible_with_multiple_actions(self):
        """Test that KMS key with public principal and multiple specific actions is flagged"""
        # Generate KMS Client
        kms_client = client("kms", region_name=AWS_REGION_US_EAST_1)
        # Create KMS key with public policy granting multiple actions
        key = kms_client.create_key(
            MultiRegion=False,
            Policy=json.dumps(
                {
